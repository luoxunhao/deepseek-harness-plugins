...
/** Normalize a candidate path to forward slashes for separator-agnostic probes. */
...
describe('bashCandidates', () => {
  it('derives Git Bash roots from the git executable path', () => {
    ...
    const candidates = bashCandidates({}, '/opt/git/cmd/git.exe').map(posix)
  ...
  it('adds the well-known environment-derived roots', () => {
    ...
    const candidates = bashCandidates(env, undefined).map(posix)
