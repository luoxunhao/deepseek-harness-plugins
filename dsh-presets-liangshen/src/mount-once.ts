/**
 * Host single-instance guard for the dsh-presets-liangshen plugin. A standalone
 * install of the same package mounted side by side with a family bundle would
 * otherwise re-register the same system-prompt sections and fail the boot.
 * mountOnce makes the second host apply a no-op for the lifetime of the first
 * instance (the browser half is already deduped by package name in the client
 * module host).
 *
