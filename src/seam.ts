/**
 * Seam wiring: route `ctx.sandbox.confine` through the codex-project
 * multi-root runner for sessions inside a multi-root codex project. The
 * wrapping is the plugin's only touch on the core sandbox service — the
 * core's `runnerCommand` config surface is left alone (its static argv
 * cannot carry a dynamically resolved runner path), and everything else the
 * plugin does (config CRUD, UI) composes through its own services.
 *
 * Routing decision per confine call (pure superset of the core seam):
 *  - non-Windows, non-`workspace-write`, or a workspace outside every
 *    multi-root space → the ORIGINAL confine (core behavior, bit-identical);
 *  - otherwise → the dsh-codex-project runner with the bwrap workspace-write
 *    profile argv (the exact shape the core `bwrapProfileArgs(policy)` builds
 *    for workspace-write — the wrapper parses it back; keep the two in
