/**
 * tsdown build for dsh-mcp-panel: the host-half lib (lib/index.js, ESM
 * node) plus one browser client bundle (lib/client.js, CJS closure factory
 * registering with the package-name id `@luoxunhao/dsh-mcp-panel` —
...
function clientBundle(): UserConfig {
  return {
    ...
    outputOptions: {
      ...
      banner: 'window.__ModuleLoader__.load({ id: "@luoxunhao/dsh-mcp-panel", factory: (require) => {',
