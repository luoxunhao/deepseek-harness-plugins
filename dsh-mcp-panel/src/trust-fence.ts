/**
 * Browser-trust fence for the mcp-panel routes, behaviorally identical to
 * the /api gateway's fence in @deepseek-ai/dsh-client-connection
 * (copied because the package does not export these helpers and the plugin
 * must not depend on its internals). Host-header loopback or a configured
 * trusted authority passes; cross-site browser markers refuse.
...
/**
 * Decide whether one mcp-panel request may reach the plugin routes.
